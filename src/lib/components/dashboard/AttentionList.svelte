...
<ul class="divide-y divide-border">
	{#each items as item (item.text)}
		<li>
			<a
				href={item.href}
				class="flex items-center gap-3 px-5 py-3.5 transition-colors hover:bg-background"
			>
				<span
					class={cn(
				...
				<span class="flex-1 text-sm text-foreground">{item.text}</span>
				<ArrowRightIcon class="h-4 w-4 shrink-0 text-muted-foreground" aria-hidden="true" />
			</a>
		</li>
	{/each}
</ul>
